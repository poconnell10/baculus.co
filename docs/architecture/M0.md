...
> closure proof), and `scripts/verify_massive.py` (live Massive vendor proof —
> AAPL daily, Jan 2020, adjusted; non-destructive, isolated per `proof_run_id`;
> fails closed without `MASSIVE_API_KEY`).
...
Two identities are tracked per raw artifact (see ADR-0010):

- **`raw_sha256`** — SHA-256 of the exact vendor bytes (raw provenance +
  content address).
- **`logical_data_sha256`** — hash of the canonical market observations only
  (symbol, event_date, OHLC, volume), **excluding** volatile request/transport
  metadata (request ids, generated timestamps, trace/pagination metadata).

Economic vintaging and restatement key on the **logical** identity, never on raw
bytes alone. The logical key for a range is `source | dataset | event_date_min |
event_date_max`. Classification:

- **Identical refetch** (same raw bytes) — no new `raw_artifact`, object not
  rewritten; the run + a new `raw_observation` are recorded plus an
  `identical_refetch` **audit event**. Operational, deliberately not governance.
- **Raw-only change** (different raw bytes, **same** logical data) — a new
  `raw_artifact`/observation is stored for exact provenance, but this is **not**
  a restatement and creates **no** new economic vintage. Recorded as a
  `raw_metadata_changed_same_logical` **audit event**, not governance.
- **Restatement** (different raw bytes, **different** logical data) — the prior
  artifact/vintage is preserved, a new **economic vintage** is created, and
  `RESTATEMENT_DETECTED` + `NEW_VINTAGE_CREATED` governance events are raised.
  The restatement event names exactly which `(symbol, event_date)` observations
  changed. History is never overwritten.

Multiple raw artifacts may share one economic vintage (raw-only changes), so raw
identity is unique via `(source_id, sha256)` while `(logical_key, vintage)` is
not (migration `0007`).

## 5. Massive adapter boundary

All Massive-specific behavior (endpoints, auth header, field names `t/o/h/l/c/v`,
...
- **FIXTURE** — deterministic, offline; used for the synthetic M0 proof cohort.
- **LIVE** — calls the real Massive REST API via the header-only, redacting
  `MassiveHTTPClient` (Bearer `MASSIVE_API_KEY`). It fetches the daily
  aggregates endpoint for a single ticker/range, follows `next_url` pagination,
  and retains every page verbatim inside a content-addressable envelope. The key
  travels only in the request header and is never logged, echoed into request
  metadata, or stored. Full retry/backoff/timeout handling surfaces
  machine-readable `MASSIVE_*` reason codes; vendor/identity/integrity defects
  fail closed (quarantine) before any canonical propagation.

Fixture-derived evidence is always tagged `FetchMode.FIXTURE` so it can never be
mistaken for live-vendor proof.

> **Contract grounded, live re-confirmation pending (ADR-0009).** The `t/o/h/l/c/v`
> mapping, `/v2/aggs` daily endpoint, `/v3/reference/tickers` lookup, ET
> business-date derivation, `Decimal(18,8)` prices, and `next_url` pagination are
> grounded in Massive's official client (`massive-com/client-python`, a
> polygon-api-client fork). The residual live-only facts (exact host, rate limits,
> whether the vendor restates history) are re-confirmed by
> `scripts/verify_massive.py` and recorded in ADR-0009 before live output backs
> any sealed claim.
...
> **Migration governance (ADR-0011).** Migrations use the Supabase timestamp
> convention (`<YYYYMMDDHHMMSS>_name.sql`). **Persistent** remote databases are
> migrated **only** via `supabase db push` and tracked in
> `supabase_migrations.schema_migrations` — never by direct `psql`/`apply_migrations`.
> **Local/CI ephemeral** Postgres may replay the same files from zero
> (`apply_migrations`, CI psql loop) for reproducibility only; it establishes no
> remote state. Applied migrations are immutable — new changes require a new
> migration file.
...
- additionally at the database level via the `dataset_builds` seal-guard trigger
  (`supabase/migrations/20260817215822_seal_guard.sql`), verified in CI.
...
- **Live Massive is not yet proven end-to-end.** The full LIVE path is
  implemented and exercised offline against a scripted transport (real HTTP
  client, retries, pagination, envelope retention, normalization, validation,
  quarantine, replay/restatement — see `engine/tests/massive/`), but
  `MASSIVE_API_KEY` was not available in this environment, so all committed
  evidence is fixture/mock-based and tagged as such. `scripts/verify_massive.py`
  runs the real AAPL/Jan-2020/adjusted proof once a key is provided.
- **Massive vendor semantics are grounded but not live-verified (ADR-0009).** The
  contract (endpoints, auth, field mapping, ET business date, pagination) is
  grounded in the vendor client; the residual live-only facts are re-confirmed by
  `scripts/verify_massive.py`. This is a gate for M0 Integration PASS.
